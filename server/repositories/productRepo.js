...
const getProduct = async (param, field) => {
  try {
    ...
    const dataReturn = data.map((item) => {
      ...
      tmp_obj.description = item.description ? item.description : "";
      tmp_obj.category = item.category ? item.category.name : "";
      tmp_obj.comments = Array.isArray(item.comments) ? item.comments : [];
      // Nếu như filed == _id thì trả thêm ảnh của phẩn tử đầu tiên vì mảng trả về chỉ có 1 phần tử.
      if (field == "_id") {
  ...
  try {
    ...
    if (Array.isArray(images)) {
      ...
      for (const element of images) {
        let dataReturn = await imageRepo.createImage(
          element.url,
          element.caption,
          element.size
        );
      ...
      imgData = imageArray.map((element) => {
        return {
          _id: element._id,
          url: element.url,
        };
